...
cover_genome_figures <- function(species_name,titre1,titre2) {
  ...
  dv.wing.cutoff=dv.wing[which(dv.wing$value < 300),]
  ...
  p_depthcov=ggplot() +
    geom_rect(data=rectangles.toprint, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax+200), 
              fill='gray80', alpha=0.8)+  
    ...
    xlab("Base position (Mb)")+ylab("Depth coverage")+ggtitle(titre2)+ylim(0, 300)+
    scale_x_continuous( labels=function(x) round(x/2.5,digits=0) )+
    theme_classic()
